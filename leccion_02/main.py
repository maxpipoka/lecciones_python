class VehicleInfo:
    ...
    def __init__(self, brand, electric, catalogue_price):
        ...
        if catalogue_price < 0:
            raise ValueError(f"Catalogue price should be a positive number, but received {catalogue_price} instead.")
    ...
    def can_lease(self, year_income: int) -> bool:
        if year_income < 0:
            raise ValueError(
                f"Year Income should be a positive number, but received {year_income} instead."
            )
